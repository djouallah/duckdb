...
Select SETTLEMENTDATE,date(SETTLEMENTDATE) as date,DUID, sum(mwh) as mwh from  xx group by all order by SETTLEMENTDATE desc
''').arrow()
...
c = alt.Chart(results).mark_area().encode( x='date:T', y='mwh:Q',color='DUID',
